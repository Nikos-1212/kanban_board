            ...
            children: [
              Row(
                ...
                children: [
                    ...
                    const Expanded(
                    flex: 1,
                    child: Text(AppConst.addNewTicket,style: TextStyle(fontSize: 18),textAlign: TextAlign.center,)),
                    Expanded(
                    flex: 0,
        ...
        Row(
            ...
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color:res.appModel.isLightTheme? Colors.black54:AppColors.whiteColour,width: 0),
                  // color: AppColor.grey10,
                ),
                child: 
                 BlocSelector<TaskBloc, TaskBlocState, int>(
          selector: (state) {
            if(state is TaskBlocInitial)
            {
          ...
          builder: (context, numOfDay) {
            return  DropdownButtonFormField(              
                  ...
                  value:numOfDay,//numOfDay==0?AppConst.selectDay:'$numOfDay',
                ...
                return GradientPrimaryButton(
                  ...
                  onPressed: () {
                    ...
                    if(state is TaskBlocInitial)
                    {
                           ...
                           final tsk=DataList(
                s: context.read<TaskBloc>().controller.getGroupController('$index')?.groupData.items.length.toString(),
                cat: '$index',
                titile: state.taskModel.title??'', //textfield
                createdDate: DateTime.now().millisecondsSinceEpoch,
                description:  state.taskModel.description??'',//textfield
                images: [],
                comments: [Comment.fromMap({
                          "text":state.taskModel.comments??'',
                          "dt":DateTime.now().millisecondsSinceEpoch
                      })],//textfield
                durationOfTask: DurationOfTask.fromMap({ "in_progress":0,
                "completed":0}),
                lastDate: DateTime.now().add(Duration(days:state.taskModel.numOfDay??0)).millisecondsSinceEpoch,
